...
from src.fsc_dataloader_2 import get_dataset,prepare_dataset,collate_fn
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--output_denoising_lora",type=str,default="subject_fill_finetuned_union",) 
    ...
    parser.add_argument("--dataset_name",type=str,default=[
            "/mnt/disk2/aiotlab/hachi/Data/ObjectStitch_V2",
        ],
    )
...
def main(args):
    ...
    text_encoder_one = text_encoder_one.to("cpu", dtype=weight_dtype)
    text_encoder_two = text_encoder_two.to("cpu", dtype=weight_dtype)
    ...
    # Keep on CPU until after LoRA adapters are loaded — avoids
    # paying the full ~24 GB static cost before we've trimmed it.
    ...
    # Move transformer to GPU only after all adapters are configured.
    # Loading LoRAs onto a CPU model and then moving to GPU in one shot
    # is far cleaner than loading onto an already-resident GPU model.
    ...
    # Use 8-bit AdamW to cut optimizer state memory (~halves it).
    # Falls back to regular AdamW if bitsandbytes isn't functional.
    try:
        ...
        logger.info("bitsandbytes unavailable, using standard AdamW optimizer.")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with torch.no_grad():
                ...
                # Save shape info before we delete latent_image
                lat_h, lat_w = latent_image.shape[2], latent_image.shape[3]
                lat_c = latent_image.shape[1]
                ...
                # target for loss = noise - latent_image; compute and store now before deletion
                target = noise - latent_image

                packed_noisy_model_input = FluxPipeline._pack_latents(
                    noisy_model_input,
                    batch_size=bsz,
                    num_channels_latents=lat_c,
                    height=lat_h,
                    width=lat_w,
                )
                # Free large intermediates we no longer need before the forward pass
                del noisy_model_input, latent_image, noise
                for i, images_per_condition in enumerate(condition_latents):
            ...
            with accelerator.accumulate(transformer):
                ...
                # target was precomputed as (noise - latent_image) before deletion
