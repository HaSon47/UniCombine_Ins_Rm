"""
Chỉnh lại cơ chế CMMDiT Attention
C_subject có thể chú ý đến C_fill
"""
print("------------use new infer------------")
import ipdb
import torch
...
def attn_forward(
    ...
    if condition_latents is not None and len(condition_latents) > 0:
        ...
        for i, cond_type in enumerate(condition_types):
            ...
            cond_query_list.append(cond_query)
            cond_key_list.append(cond_key)
            cond_value_list.append(cond_value)
        ...
        for i, cond_type in enumerate(condition_types):
            ...
            cond_query = cond_query_list[i]
            cond_key = cond_key_list[i]
            cond_value = cond_value_list[i]

            if cond_type == "subject":
                cf_idx = condition_types.index("fill")
                cf_key = cond_key_list[cf_idx]
                cf_value = cond_value_list[cf_idx]

                mix_cond_key = torch.cat([base_key, cond_key, cf_key], dim=2)
                mix_cond_value = torch.cat([base_value, cond_value, cf_value], dim=2)
